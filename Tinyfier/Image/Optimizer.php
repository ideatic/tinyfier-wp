...
abstract class Tinyfier_Image_Optimizer {
    ...
    private static function _find_tool($name, $local_search = TRUE, $throw_not_found = TRUE) {
        ...
        if ($throw_not_found) {
            throw new RuntimeException("$name is missing, optimization cannot be done");
        } else {
            return FALSE;
        }
    }
    ...
    private static function _optimize_png($file, $settings, $commad_prefix) {
        ...
        if ($settings[self::MODE] == self::MODE_LOSSY) {
            ...
            $tmp = tempnam(sys_get_temp_dir(), 'pngquant');
            copy($file, $tmp);
            self::_exec("{$commad_prefix}{$pngquant} --force --speed {$levels[$settings[self::LEVEL]]} --quality={$min_quality}-{$max_quality} --ext _pq.png :file", array(
                ':file' => $tmp
                    ), $settings);
            $ext = pathinfo($tmp, PATHINFO_EXTENSION);
            $out = dirname($tmp) . '/' . str_replace(".$ext", '_pq.png', basename($tmp));
            if (file_exists($out)) {
                rename($out, $file);
            } else {
                $out = dirname($tmp) . '/' . basename($tmp) . '_pq.png';
                if (file_exists($out)) {
                    rename($out, $file);
                }
            }
